...
- `--output-dir`: default is current working directory
...
```bash
uv run ./tools/release-pipeline commits   --repo {owner/repo} --tag {vX.Y.Z} [--remote {remote}] -o commits.json
...
Read `prs_categorized.md` and present the Summary table to the user. Then:
...
```bash
uv run ./tools/release-pipeline research \
  --repo {owner/repo} \
...
Read `research.json` and write `{output-dir}/blog_post_{version}.md`.

**Structure:**
```
# Argo CD vX.Y Release Candidate
*By the Argo CD Community*
---
[Intro: 2 paragraphs]
...
**Per-feature formula** (75–150 words; up to 200 for major features):
...
3. **How to use** — config snippet, CLI example, or annotation — whatever is actionable

**Style:**
- Tone: enthusiastic and community-oriented ("The community is thrilled to announce...")
- H2 per section, H3 for sub-features within a grouped section
- Inline links: `[PR #N](url)` and `[#N](issue url)` embedded naturally in prose — never bare `#NNNNN`
...
| Any `cherry-pick/*` label (1st) | +15 |
| 2+ `cherry-pick/*` labels | +10 more |
| `component:*` label | +10 |
| `type:scalability` or `type:tech-debt` | +5 |
